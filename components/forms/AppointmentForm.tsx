...
export function AppointmentForm() {
  ...
  // Cache compressed image data so img2img reuses it (no re-compression needed)
  const [compressedPhotoData, setCompressedPhotoData] = useState<{
    base64: string
    mimeType: string
  } | null>(null)
  ...
  useEffect(() => {
    if (
      ...
      // Reuse already-compressed photo — no second compression needed
      editGroomingPhotoAction(
        style.id,
        style.name,
        style.dallePrompt,
        compressedPhotoData.base64,
        compressedPhotoData.mimeType,
      )
        .then((result) => {
          if (result.data) {
  ...
  }, [suggestedStyles, compressedPhotoData, geo.status, generatingStyleId, groomingPreviews])
  ...
  const handlePhotoChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!allowed.includes(file.type)) {
        setAiError(
          'Solo se aceptan imágenes JPG, PNG, WebP o HEIC.',
        )
        return
      }
